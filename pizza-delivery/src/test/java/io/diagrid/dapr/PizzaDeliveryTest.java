...
public class PizzaDeliveryTest {
    ...
    public void testDelivery() throws Exception {
        ...
        assertEquals("Four published event are expected",6, events.size());
        ...
        assertEquals("The content of the cloud event should be the order-out-on-its-way event", EventType.ORDER_ON_ITS_WAY, events.get(3).getData().type());
        assertEquals("The content of the cloud event should be the order-out-on-its-way event", EventType.ORDER_ON_ITS_WAY, events.get(4).getData().type());
        assertEquals("The content of the cloud event should be the order-completed event", EventType.ORDER_COMPLETED, events.get(5).getData().type());
